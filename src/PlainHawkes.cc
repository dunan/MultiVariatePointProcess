...
void PlainHawkes::PlotIntensityFunction(const Sequence& data, const unsigned& dim_id)
{
	double delta = 0.01;
	...
	Eigen::Map<Eigen::MatrixXd> Alpha = Eigen::Map<Eigen::MatrixXd>(parameters_.segment(num_dims_, num_dims_ * num_dims_).data(), num_dims_, num_dims_);
	...
	{
		Eigen::VectorXd y_i = t.array() - i_event->time;
		
		y_i = (y_i.array() < 0).select(0, y_i);

		const unsigned& m = i_event->DimentionID;

		y_i = ((-Beta_(m, dim_id)) * y_i.array()).exp();
		y_i = (y_i.array() == 1).select(0, y_i);
		y_i = y_i.array() * Alpha(m, dim_id);	
