...
void HawkesLearningTriggeringKernel::fit(const std::vector<Sequence>& data, const OPTION& options)
{
	...
	switch (options_.excitation_regularizer)
	{
		case GROUP :
			...
			opt.ProximalGroupLassoForHawkes(1e-1, options_.coefficients[LAMBDA], 500, num_rbfs_);
		...
		for(unsigned c = 0; c < num_sequences_; ++ c)
		{
			...
			if(nc > 0) // if infected
			{
				for(unsigned k = 0; k < nc; ++ k)
				{
					...
					double intensity_ck = Lambda0_(i) + (arrayK[i][c][k] * MatrixAlpha[i]).trace();
...
double HawkesLearningTriggeringKernel::IntensityUpperBound(const double& t, const Sequence& data, Eigen::VectorXd& intensity_upper_dim)
{
	return 0;
}
